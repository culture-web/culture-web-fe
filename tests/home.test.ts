...
test('Clicking on Learn More About Other Cultures brings us to cultures', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('button', { name: 'Learn More About Other Cultures' })).toBeVisible();
  await page.getByRole('button', { name: 'Learn More About Other Cultures' }).click();
...
test('Clicking on Learn More About KathakalAI brings us to About Us', async ({ page }) => {
  await page.goto('/');
  await expect(
    page.getByRole('button', { name: 'Learn More About KathakalAI' }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Learn More About KathakalAI' }).click();
